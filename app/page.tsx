...
const removeEvento = async (id: string) => {
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-zinc-100 font-sans selection:bg-amber-500 selection:text-black overflow-x-hidden">
      {/* Estilos globais injetados diretamente para compatibilidade */}
      <style>{`
  ...
  <div className="container mx-auto px-6">
    <div className="text-center mb-16">
      ...
      <h2 className="text-3xl md:text-5xl font-display text-white">Agenda</h2>

      {/* Painel Admin com inputs para adicionar evento */}
      {adminMode && (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {agenda.length === 0 ? (
        <p className="text-zinc-400 col-span-full text-center">Nenhum evento cadastrado no momento.</p>
      ) : (
        agenda.map((evento) => (
          <div key={evento.id} className="bg-zinc-800 border border-white/5 p-6 rounded-lg hover:border-amber-500 transition-all relative">
            <h3 className="text-xl font-display text-white mb-2">{evento.title}</h3>
            <p className="text-zinc-400 text-sm mb-1"><strong>Data:</strong> {evento.date}</p>
            <p className="text-zinc-400 text-sm"><strong>Local:</strong> {evento.location}</p>

            {adminMode && (
              <button 
                onClick={() => removeEvento(evento.id)} 
                className="absolute top-2 right-2 text-red-500 text-xs border border-red-500 px-2 py-1 rounded hover:bg-red-500 hover:text-black transition-colors"
              >
                Excluir
              </button>
            )}
      ...
      <section id="services" className="py-24 bg-zinc-900/30 border-y border-white/5">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {/* Card 1 - ALINHAMENTO RESPONSIVO APLICADO */}
